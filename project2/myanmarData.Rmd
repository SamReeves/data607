...
file1 <- "mvoter_app.json"
file2 <- "bo-disclosure.csv"
```
...
bodisc <- read.csv("bo-disclosureFIXED.csv")
```
...
At first, I thought that row 26 was the problem, with unmatched quotes damagind the overall structure.  Omitting that row with two reads and an rbind() call fixed nothing.

We have so many issues, it's hard to know what they all are... So, let's try just drawing some of the columns.

```{r}
bodisc <- read.csv("bo-disclosureFIXED.csv", header=TRUE, nrows = 1)
cols <- names(bodisc)[1:100]
```
...
```{r}
bodisc <- read.csv("bo-disclosureMARRED.csv", header = TRUE, fill = TRUE)
bodisc[1:10, 1:10]
```
